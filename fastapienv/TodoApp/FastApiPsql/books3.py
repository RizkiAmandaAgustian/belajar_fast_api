...
from TodoApp.FastApiPsql import auth
...
@router.post('/test_langsungtanpaorm1')
async def tes_tanpa_orm (user : user_dependency, title: Optional [str] = Form(None,min_length=3) , description: Optional [str] = Form(None,min_length=3,max_length=100), priority: Optional [int]= Form(None,gt=0, lt=6), complete: Optional[bool] = Form(False)):
    tes = models.create_barang(title,description,priority,complete,user['id'])
    return tes, status.HTTP_200_OK

# @router.post('/test_langsungtanpaorm', status_code=status.HTTP_201_CREATED)
# async def tes_tanpa_orm(
#     user: user_dependency,
#     title: Optional[str] = Form(None, min_length=3),
#     description: Optional[str] = Form(None, min_length=3, max_length=100),
#     priority: Optional[int] = Form(None, gt=0, lt=6),
#     complete: Optional[bool] = Form(False)
# ):
#     try:
#         create_barang(title, description, priority, complete, user['id'])
...
'''COBA PERTAMA TANPA ORM DIATAS INI
db_config = {
...
def create_barang(title: str, description: str, priority: int, complete: bool):
    with conn.cursor() as cursor:
        try:
            cursor.execute(
                'INSERT INTO todos (title, description, priority, complete) VALUES (%s, %s, %s, %s)',
...
# Endpoint untuk menambahkan barang tanpa ORM
@router.post('/test_langsungtanpaormAI', status_code=status.HTTP_201_CREATED)
async def tes_tanpa_orm(
    title: Optional[str] = Form(None, min_length=3),
    description: Optional[str] = Form(None, min_length=3, max_length=100),
    priority: Optional[int] = Form(None, gt=0, lt=6),
    ...
    try:
        create_barang(title, description, priority, complete)
        return {"message": "Barang berhasil ditambahkan"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))'''
